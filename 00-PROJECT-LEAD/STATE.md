...
- Security/deploy: `require_api_key` fails open without `OWNER_API_KEY`; `api-deploy.yml` shifts traffic to `LATEST`, not the tested revision; Terraform has drifted from live (refresh job digest, timeout, memory). `terraform-ci` (the sole WIF-bound service account for every GitHub Actions workflow, found 2026-09-19 while building `pipeline-deploy.yml`) holds `roles/editor` **and** `roles/iam.securityAdmin` project-wide — Terraform apply genuinely needs broad access, but the same identity now also builds/pushes every image and updates/runs Cloud Run jobs and services for three separate deploy workflows. `iam.securityAdmin` is the sharper edge: it lets this identity modify project IAM policy, including granting itself more access, so a compromised PR or leaked WIF trust on this repo is a path to persistent owner-level control, not just "can redeploy things." Not urgent — nothing found exploiting it, and every workflow that uses it is repo-scoped by the WIF attribute condition — but worth splitting eventually: a narrow deploy-only SA (`roles/run.admin` + `roles/artifactregistry.writer`, no `securityAdmin`) for the three deploy workflows, `terraform-ci`'s current broad grant kept only for actual `terraform apply` runs.
